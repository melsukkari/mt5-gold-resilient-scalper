...
class MT5Connector:
    ...
    def calculate_lot_size(self, sl_distance_points: float) -> float:
        ...
        try:
            ...
            logger.info(f"Calculated lot size: {lot_size} (risk={risk_amount:.2f}, sl_points={sl_distance_points})")
            return lot_size
    ...
    def place_order(
        ...
        try:
            ...
            type_name = "BUY_STOP" if order_type == mt5.ORDER_TYPE_BUY_STOP else "SELL_STOP"
            ...
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                self._last_error = f"Order failed: {result.comment} (retcode={result.retcode})"
                logger.error(self._last_error)
                return (False, None)
            
            logger.info(f"Order placed successfully: {type_name} {volume} lots at {price}, SL={sl}, TP={tp}")
    ...
    def is_connected(self) -> bool:
        ...
        return self.connected and mt5.terminal_info().connected if mt5.terminal_info() else False
