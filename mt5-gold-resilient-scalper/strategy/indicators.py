...
class TechnicalIndicators:
    ...
    def fetch_rates(
        self, timeframe: int, num_bars: int = 200
        ...
        try:
            rates = mt5.copy_rates_from_pos(
                config.symbol.SYMBOL, timeframe, 0, num_bars
            )
    ...
    def calculate_ema(
        self, df: pd.DataFrame, period: int, column: str = "close"
    ) -> Optional[pd.Series]:
        """
    ...
    def calculate_bollinger_bands(
        self, df: pd.DataFrame, period: int = 20, deviation: float = 2.0
    ...
    def get_all_indicators(
        self, entry_timeframe: int, context_timeframe: int
        ...
        try:
            ...
            bb = self.calculate_bollinger_bands(
                df_entry, config.indicators.BB_PERIOD, config.indicators.BB_DEVIATION
            )
