...
from .market_filter import MarketFilter, MarketState
...
class GridManager:
    ...
    def execute_strategy_cycle(self) -> Dict[str, any]:
        ...
        try:
            ...
            position_count = len([p for p in positions if p.magic == config.trading.MAGIC_NUMBER])
            ...
            if bias == 'bullish':
                ...
                success = self._place_pending_order(
                    ...
                    sl=levels['buy_sl'],
                    tp=levels['buy_tp'],
                    lot_size=self._calculate_lot_size(levels['sl_distance_points'], account_info['balance'])
                )
                if success:
                    orders_placed += 1
    ...
    def _place_pending_order(
        ...
        try:
            ...
            # Place new order
            order_type_name = "BUY_STOP" if order_type == mt5.ORDER_TYPE_BUY_STOP else "SELL_STOP"
